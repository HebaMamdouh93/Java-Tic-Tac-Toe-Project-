...
public  class computerfinalBase extends AnchorPane  {
 ...
 public  static String  status; // status the game
   ...
   public static ArrayList <Integer> stepsplayer=new ArrayList<>(); //list have all steps move for the game
...
arrayButtons[8]=button9;
  ...
  button0.setOnAction(new EventHandler<ActionEvent>(){
     ...
     @Override
     public void handle(ActionEvent event) {
         if(position.board[indx_b1]==' ') //check if the position of this bord empty so i can play on it this position
         {
     
         if(!position.isGameEnd()) //check if the game is not end to can player make the move
         {
  ...
  Image img = new Image(getClass().getResourceAsStream("x.png"));
         ...
         else
         {
             ...
             if(!position.isGameEnd()) // check again if the game not end to can computer play his move 
             {
